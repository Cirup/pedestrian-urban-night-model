...
to setup-remaining-streetlights
  ...
  ; reate the remaining streetlights
  create-streetlights remaining-streetlights [
    ; Find a random sidewalk patch to place the streetlight
  ...
  ; Update the current number of streetlights
...
to setup-one-streetlight
  ...
  ifelse current-number-of-streetlights < number-of-streetlights [
    ; Check if the clicked patch is a sidewalk
    ifelse [is-sidewalk?] of clicked-patch [
      ; Create a new streetlight on the clicked patch
      create-streetlights 1 [
        set shape "circle"
        set color grey - 2
        set size 1
        move-to clicked-patch
        set light brightness  ; Set the light attribute to the desired brightness
      ]
