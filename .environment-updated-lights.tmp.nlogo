...
pedestrians-own [
  ...
  fear-tolerance; determines fear threshold a pedestrian can take to walk in that
]
...
to generate-field
  set light-level 0
  ; every patch needs to check in with every streetlight
  ask streetlights [
    set-field myself
  ]

  if model-version = "light-visualization" [
...
to draw-road
  ...
  ask patches with [abs pycor <= number-of-lanes and not is-road? and not is-grass?] [
    set is-sidewalk? true
  ]
...
to random-rotate [patches-in-vision]
  ...
  let road-patches count patches-in-vision with [is-road?]
...
to handle-stuck-agents [in-vision]
